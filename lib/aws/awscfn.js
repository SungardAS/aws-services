...
function AWSCfn() {
  ...
  me.createCfnStack = function(input, callback) {
    var self = arguments.callee;
    param = []
    for (var key in input.params) {
       param.push({ParameterKey:key,ParameterValue:input.params[key]})
    }
    var cnfparam = {
        StackName: input.stackName,
        Capabilities: ['CAPABILITY_IAM'],
  ...
  me.updateCfnStack = function(input, callback) {
      var self = arguments.callee;
      param = []
      for (var key in input.params) {
          param.push({ParameterKey:key,ParameterValue:input.params[key]})
      ...
      var cnfparam = {
          ...
          Capabilities: ['CAPABILITY_IAM'],
          Parameters: param,
          TemplateURL: input.s3Url
      };

      var cfnService = me.findService(input);
      if (callback) {
          var cfnService = me.findService(input);
          cfnService.updateStack(cnfparam, callback);
          return;
      }
      self.callbackFind = function(data) {
          return data;
      ...
      var cfnService = me.preRun(self, input);
      cfnService.updateStack(cnfparam, me.callbackFind);
  }
