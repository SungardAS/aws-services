...
function AWSConfig() {
  ...
  me.sendEvaluations = function(input, callback) {
    var COMPLAINT_TYPE = "COMPLIANT",
      NON_COMPLIANT_TYPE = "NON_COMPLIANT",
      annotation = "",
      complianceType = COMPLAINT_TYPE,
      rules = input.rules;
    ...
    for (var idx in rules){
      ...
      if (ingressPerm.length !==0){
          ...
          complianceType = NON_COMPLIANT_TYPE;
      }
      ...
      if (callback) {
          var configService = me.findService(input);
          configService.sendEvaluation(params, callback);
          return;
      }
      ...
      var configService = me.preRun(self, input);
      configService.sendEvaluation(params, me.callbackFind);
    }
  }
  // {}

  me.sendEvaluation = function(input, callback) {

    var params = {
        Evaluations: [
            {
                ComplianceResourceType: input.resourceType,
                ComplianceResourceId: input.resourceId,
