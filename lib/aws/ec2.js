...
function AWSEC2() {
   ...
   me.getEc2List = function(input, callback) {
      var self = arguments.callee;
      var ec2Service = me.findService(input);
      var params = {Filters: [{Name: 'tag-key',Values: ['Managed']}]};
      if (callback) {
         var ec2Service = me.findService(input);
      ...
      self.callbackFind = function(data) {
		...
		if(tags[k].Key == 'Name'){
              ...
              console.log(tagName)
              var opt = {
                keyName:instances[j].KeyName,
                instanceId:instances[j].InstanceId,
                instanceType:instances[j].InstanceType,
                status:instances[j].State.Name,
                privateIpAddress:instances[j].PrivateIpAddress,
                region:input.region, 
      ...
      var ec2Service = me.preRun(self, input);
      ec2Service.describeInstances(params, me.callbackFind);
  }
