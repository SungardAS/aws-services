  ...
  me.readObject = function(input, callback) {
    ...
    me.updatePolicy = function(input, callback) {
        ...
        s3.getBucketPolicy(params, function(err, data) {
            if (err) {
                callback("Error updating Policy", null);
            }
            else {
                if(data.Policy === undefined) me.addPolicy(input, callback);
                else{
                    var accounts = data.Policy.Statement[0].Principal.AWS;
                    accounts.push(input.account);
                    input.policyDocument = JSON.stringify(data.Policy);
                    console.log("Policy changed to ", input.policyDocument);
                    me.addPolicy(input, callback);
                }
            }
        });
    }
