...
function GMail() {
  ...
  me.authorize = function(input, main_function, callback) {
    ...
    fs.readFile(TOKEN_PATH, function(err, token) {
      if (err) {
        me.getNewToken(oauth2Client, main_function, callback);
      } else {
        oauth2Client.credentials = JSON.parse(token);
        main_function(oauth2Client, input, callback);
      }
    });
  }

  me.getNewToken = function(oauth2Client, main_function, callback) {
  ...
  function findLabels(auth, input, callback) {
    ...
    gmail.users.labels.list(params, function(err, response) {
      if (err) {
        console.log("Error in gmail.users.labels.list : " + err, err.stack);
        fc.run_error_function(me.listLabels, err);
        return;
      }
      var labels = response.labels;
      if (labels.length == 0) {
        console.log('No labels found.');
        fc.run_failure_function(me.listLabels, input);
      }
      else {
        console.log('Labels:');
        ...
        input.labels = labels;
        fc.run_success_function(me.listLabels, input);
      }
    });
  }
  ...
  function findMessages(auth, input, callback) {
    var gmail = google.gmail('v1');
    var params = {
      auth: auth,
      userId: input.userId,
    ...
    gmail.users.messages.list(params, function(err, response) {
      if (err) {
        console.log("Error in gmail.users.messages.list : " + err, err.stack);
        fc.run_error_function(me.listMessages, err);
        return;
      }
      var messages = response.messages;
      if (messages.length == 0) {
        console.log('No messages found.');
        fc.run_failure_function(me.listMessages, input);
      }
      else {
        ...
        fc.run_success_function(me.listMessages, input);
      }
    });
  }

  me.getMessage = function(input, callback) {
    me.authorize(input, findMessage, callback);
  }

  function findMessage(auth, input, callback) {
    ...
    gmail.users.messages.get(params, function(err, response) {
      if (err) {
        console.log("Error in gmail.users.messages.list : " + err, err.stack);
        fc.run_error_function(me.getMessage, err);
        return;
      }
      ...
      input.currentMessage = response;
      fc.run_success_function(me.getMessage, input);
    });
  }
  /* { id: '14ef4ff9733bf509',
