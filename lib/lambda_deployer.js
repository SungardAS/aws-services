...
function LambdaDeployer() {
  ...
  function addFederateAssumeRolePolicy(input) {
    if (input.federateRoleName) {
      input.lambdaRoleArn = input.roleArn;
  ...
  me.clean = function(input, callback) {
    ...
    var flows = [
      {func:aws_sts.assumeRoles, success:aws_role.findRoleByPrefix},
      {func:aws_role.findRoleByPrefix, success:removeFederateAssumeRolePolicy, failure:aws_lambda.findFunction},
      ...
      {func:aws_role.findInlinePolicy, success:aws_role.deleteInlinePolicy, failure:callback},
  ...
  function readPackage(dirName, packageJSONFileName) {
    ...
    var input = {
      ...
      roleNamePostfix: (new Date()).getTime().toString(),
      assumeRolePolicyDocument: JSON.stringify(packageJSON.assumeRolePolicyDocument),
      inlinePolicyName: packageJSON.inlinePolicyName,
      inlinePolicyDocument: JSON.stringify(packageJSON.inlinePolicyDocument),
      memorySize: packageJSON.memorySize,
      timeout: packageJSON.timeout,
      federateRoleName: packageJSON.federateRoleName
    };
    console.log(input);
  ...
  me.buildPackage = function(action, dirName, packageJSONFileName, assumeRoleInfoBeforeDeploy, callback) {
