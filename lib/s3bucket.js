...
function AWSS3Bucket() {
  ...
  me.addPolicy = function(input) {
    ...
    s3.putBucketPolicy(params, function(err, data) {
      if (err) {
        console.log("Error in putBucketPolicy : " + err, err.stack);
        fc.run_error_function(me.addPolicy, err);
      }
      else {
        console.log(data);
        console.log("succesfully added a policy");
        fc.run_success_function(me.addPolicy, input);
      }
    });
  }

  me.getPolicy = function(input) {
    if (input.profile) {
      var credentials = new AWS.SharedIniFileCredentials({profile: input.profile});
      AWS.config.credentials = credentials;
    }
    ...
    s3.getBucketPolicy(params, function(err, data) {
      if (err) {
        console.log("Error in getBucketPolicy : " + err, err.stack);
        fc.run_failure_function(me.getPolicy, input);
      }
      else {
        console.log(data);
        console.log("succesfully added a policy");
        fc.run_success_function(me.getPolicy, input);
      }
    });
  }
}
